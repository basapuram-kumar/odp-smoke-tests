#!/usr/bin/env bash
#
# Spark 3.5.1 (ODP layout spark3_3_5_1-client): kinit spark/<FQDN>, then SparkPi on YARN.
# JAR is resolved with a glob (no hard-coded full version in this script).
#
# Equivalent to:
#   SPARK_MAJOR_VERSION=3 SPARK_VERSION=3_3_5_1 spark-submit \
#     --class org.apache.spark.examples.SparkPi --master yarn \
#     /usr/odp/current/spark3_3_5_1-client/examples/jars/spark-examples_*.jar 10
#
# Environment (optional):
#   SPARK_KEYTAB, SPARK_PRINCIPAL_HOST, SPARK3_CLIENT_HOME (default .../spark3_3_5_1-client)
#   SPARK_SUBMIT, SPARK_EXAMPLES_JAR_GLOB, SPARK_PI_SLICES (default 10)
#   SPARK_MAJOR_VERSION (default 3), SPARK_VERSION (default 3_3_5_1)
#
set -euo pipefail

SPARK_KEYTAB="${SPARK_KEYTAB:-/etc/security/keytabs/spark.service.keytab}"
SPARK3_CLIENT_HOME="${SPARK3_CLIENT_HOME:-/usr/odp/current/spark3_3_5_1-client}"
SPARK_PI_SLICES="${SPARK_PI_SLICES:-10}"
SPARK_MAJOR_VERSION="${SPARK_MAJOR_VERSION:-3}"
SPARK_VERSION="${SPARK_VERSION:-3_3_5_1}"
...
echo "---- spark-submit SparkPi (YARN) Spark 3.5.1 ----"
"${spark_submit}" \
...
echo "OK: Spark 3.5.1 SparkPi smoke finished."
